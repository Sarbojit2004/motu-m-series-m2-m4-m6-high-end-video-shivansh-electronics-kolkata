...
/**
 * SECTION 6 LOGO RULE — identical to the landscape build, re-anchored.
 *
 * Every mark here is positioned against the CAPTION-SAFE ZONE (top 180 /
 * bottom 220 / side 64), not against the landscape margins, so no logo, URL or
 * contact line can land under a platform caption overlay or action rail.
 *
...
/**
 * Persistent corner mark — anchored BOTTOM-RIGHT on this canvas.
 *
 * The landscape build puts it top-right, which works at 1920 wide because a
 * headline never reaches that far. At 1080 the content box is only 952px and
 * headlines run its full width: a first QA pass had "ONE ENGINE. THREE SIZES.",
 * "ESS SABRE32 ULTRA." and "RECORD ON DAY ONE." all colliding with the mark.
 *
 * So on portrait ALL branding lives in the bottom band and headings own the
 * top. Scenes.tsx reserves the matching space, and the MOTU mark sits
 * bottom-LEFT, which leaves 952 - 200 - 300 = 452px of clearance between them.
 */
export const ShivanshCorner: React.FC<{
  ...
  width?: number;
  withUrl?: boolean;
}> = ({ delay = 12, position = "br", width = 300, withUrl = true }) => {
  const frame = useCurrentFrame();
  const t = ramp(frame, delay, 20, EASE.out);
  const vert = position.startsWith("t") ? { top: SAFE.top } : { bottom: SAFE.bottom };
  const horz = position.endsWith("l") ? { left: SAFE.marginX } : { right: SAFE.marginX };
...
export const MotuCorner: React.FC<{
  ...
  position?: "tl" | "tr" | "bl" | "br";
  width?: number;
}> = ({ delay = 12, position = "bl", width = 200 }) => {
  const frame = useCurrentFrame();
  const t = ramp(frame, delay, 20, EASE.out);
  const vert = position.startsWith("t") ? { top: SAFE.top } : { bottom: SAFE.bottom };
  const horz = position.endsWith("l") ? { left: SAFE.marginX } : { right: SAFE.marginX };
  ...
  >
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 30, textAlign: "center" }}>
      <Rise delay={delay} y={16}>
        <Logo which="shivansh" width={860} />
      </Rise>
      <Rise delay={delay + 10} y={14}>
        <div style={{ ...subhead(36, 600), color: COLORS.inkSoft, maxWidth: 900 }}>
          {BRAND.role}
        </div>
      </Rise>
      <Rise delay={delay + 16} y={12}>
        <div style={{ ...micro(28, 800, "0.14em"), color: COLORS.slate, textAlign: "center" }}>for {BRAND.region}</div>
      </Rise>
...
export const PriceLockup: React.FC<{ delay?: number; compact?: boolean }> = ({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: compact ? 20 : 30, width: "100%" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: compact ? 14 : 18, width: "100%" }}>
        {rows.map((r, i) => (
          <Rise key={r.name} delay={delay + 10 + i * 7} y={16}>
            <div
              style={{
                ...
                gap: 20,
                padding: compact ? "18px 26px" : "24px 34px",
            ...
            >
              <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "flex-start" }}>
                <span style={{ ...micro(26, 800, "0.14em"), color: COLORS.ink }}>{r.name}</span>
                <span style={{ ...micro(20, 700, "0.1em"), color: COLORS.slateDim }}>{r.io}</span>
              </div>
              <span style={{ ...spec(compact ? 56 : 64, 800, "0.005em"), color: COLORS.motuBlue, lineHeight: 1 }}>
                {r.price}
              </span>
            </div>
          </Rise>
      ...
      <Rise delay={delay + 34} y={12}>
        <div
          style={{
            ...subhead(compact ? 26 : 29, 700),
            color: COLORS.ink,
            textAlign: "center",
          }}
        >
          Visit{" "}
          <span style={{ ...spec(compact ? 27 : 30, 800, "0.02em"), color: COLORS.motuBlue }}>
            {BRAND.website}
          </span>{" "}
  ...
  >
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 26, textAlign: "center", maxWidth: 1600 }}>
      <Rise delay={delay} y={14}>
        <Logo which="shivansh" width={760} />
      </Rise>
      <Rise delay={delay + 8} y={12}>
        <div style={{ ...spec(38, 800, "0.02em"), color: COLORS.motuBlue }}>{BRAND.website}</div>
      </Rise>
      ...
      <Rise delay={delay + 26} y={12}>
        <div style={{ ...subhead(24, 500), color: COLORS.slateDim, maxWidth: 880 }}>{BRAND.address}</div>
      </Rise>
    </div>
  </AbsoluteFill>
);
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 26, textAlign: "center" }}>
      <Rise delay={delay} y={14}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 26 }}>
          <Logo which="motu" width={360} />
          <div style={{ width: 220, height: 1, background: COLORS.lineStrong }} />
          <Logo which="shivansh" width={560} />
        </div>
      </Rise>
      <Rise delay={delay + 10} y={12}>
        <div style={{ ...headline(58, 800), color: COLORS.ink }}>M2 · M4 · M6</div>
      </Rise>
      <Rise delay={delay + 16} y={12}>
        <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "center" }}>
          {[
            ...
            <div key={n} style={{ display: "flex", gap: 18, alignItems: "baseline" }}>
              <span style={{ ...micro(24, 800, "0.12em"), color: COLORS.slate, width: 190, textAlign: "right" }}>
                {n}
              </span>
              <span style={{ ...spec(48, 800, "0.01em"), color: COLORS.motuBlue }}>{p}</span>
            </div>
          ))}
        </div>
      </Rise>
      ...
      <Rise delay={delay + 32} y={10}>
        <div style={{ ...subhead(29, 600), color: COLORS.slate, maxWidth: 920 }}>
